...
watch(() => props.planData, newVal => {
  if (newVal) {
    subscriptionId.value = newVal.id
    userId.value = newVal.user.id
    planId.value = newVal.plan.id
    startDate.value = newVal.start_date
    endDate.value = newVal.end_date
    status.value = newVal.status
    amountPaid.value = newVal.amount_paid
  }
})
...
const onSubmit = () => {
  refForm.value?.validate().then(({valid}) => {
    if (valid && subscriptionId.value) {
      ...
      emit('plan-data', subscriptionId.value, formData)
      closeDrawer()
    }
  })
}
...
<template>
  ...
  >
    <AppDrawerHeaderSection title="تعديل الاشتراك" @cancel="closeDrawer"/>
    <VDivider/>
      ...
      <VCard flat>
        <VCardText>
          <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
            <VRow>
              <VCol cols="12">
                <AppSelect v-model="userId" :items="users.map(u => ({ value: u.id, title: u.name }))" label="المستخدم"
                           :rules="[requiredValidator]"/>
              </VCol>

              <VCol cols="12">
                <AppSelect v-model="planId" :items="plans.map(p => ({ value: p.id, title: p.name }))" label="الباقة"
                           :rules="[requiredValidator]"/>
              </VCol>
